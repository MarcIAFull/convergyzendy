...
export function registerGetCartTool(mcp: McpServer, reg?: ExecutorReg) {
  const handler = async (args: Record<string, unknown>) => {
    ...
    const cartItems = (items || []).map((item: { quantity: number; notes?: string; products?: { name: string; price: number }; cart_item_addons?: Array<{ addons?: { name: string; price: number } }> }) => {
      const addonsTotal = (item.cart_item_addons || []).reduce((s: number, cia: { addons?: { price: number } }) => s + (cia.addons?.price || 0), 0);
      const unitPrice = (item.products as { price: number })?.price || 0;
      const total = item.quantity * (unitPrice + addonsTotal);
      return {
        product_name: (item.products as { name: string })?.name,
        quantity: item.quantity,
        unit_price: unitPrice,
        addons: (item.cart_item_addons || []).map((cia: { addons?: { name: string; price: number } }) => cia.addons).filter(Boolean),
        total,
        notes: item.notes,
      };
    });

    const total = cartItems.reduce((s: number, i: { total: number }) => s + i.total, 0);
