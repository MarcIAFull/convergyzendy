...
export function registerGetCartTool(_mcp: any, reg?: ExecutorReg) {
  const handler = async (args: Record<string, any>) => {
    ...
    const { data: carts } = await supabase
      ...
      .eq('restaurant_id', restaurant_id).eq('user_phone', customer_phone)
      .eq('status', 'active').gt('updated_at', twelveHoursAgo)
      .order('created_at', { ascending: false }).limit(1);

    const cartId = carts?.[0]?.id;
    if (!cartId) {
      return { content: [{ type: 'text' as const, text: JSON.stringify({ success: true, items: [], total: 0, message: 'Carrinho vazio.' }) }] };
    }

    const { data: items } = await supabase
      .from('cart_items')
      .select(`id, quantity, notes, product_id, products (id, name, price), cart_item_addons (addons (id, name, price))`)
      .eq('cart_id', cartId);
    ...
    const cartItems = (items || []).map((item: any) => {
      ...
      return {
        product_name: item.products?.name, quantity: item.quantity, unit_price: unitPrice,
        addons: (item.cart_item_addons || []).map((cia: any) => cia.addons).filter(Boolean),
        total, notes: item.notes,
      };
    });
    ...
    return {
      content: [{
        type: 'text' as const,
        text: JSON.stringify({
          success: true, items: cartItems, total: Math.round(total * 100) / 100,
