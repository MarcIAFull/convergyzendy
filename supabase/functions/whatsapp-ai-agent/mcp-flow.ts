...
async function buildMinimalContext(
  ...
  if (cartId) {
    const { data: items } = await supabase
      ...
      .eq('cart_id', cartId);
    const total = (items || []).reduce((s: number, i: { quantity: number; products?: { price: number } }) =>
...
export async function runMCPFlow(params: MCPFlowParams): Promise<Response> {
  const { supabase, restaurantId, customerPhone, rawMessage, instanceName } = params;
  const corsHeaders = {
  ...
  if (!instance) {
    ...
    instance = instances?.[0]?.instance_name || '';
  }
