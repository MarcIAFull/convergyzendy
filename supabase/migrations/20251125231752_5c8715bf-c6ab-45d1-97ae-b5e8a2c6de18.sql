-- Create atomic UPSERT function for debounce queue
-- This prevents race conditions by using INSERT ... ON CONFLICT UPDATE in a single atomic operation
...
BEGIN
  ...
  ON CONFLICT ON CONSTRAINT idx_debounce_queue_unique_pending 
  DO UPDATE SET
...
-- Add comment
COMMENT ON FUNCTION public.upsert_debounce_message IS 
'Atomically insert or update a debounce queue entry, preventing race conditions when multiple messages arrive simultaneously';